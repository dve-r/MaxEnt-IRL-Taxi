...
This module implements the MaxEnt IRL algorithm (Ziebart et al., 2008) to recover 
the underlying reward function of an expert navigating the Taxi-v3 environment.
It includes forward-backward passes for computing State Visitation Frequencies (SVF)
and utilizes log-space calculations for numerical stability.
"""
...
def value_iteration(env, R_table, gamma=0.9, epsilon=0.1):
    ...
    Args:
        ...
        R_table (np.ndarray): The reward matrix of shape (S, A).
        gamma (float): Discount factor.
        epsilon (float): Convergence threshold.
        
    Returns:
        tuple: (optimal value function array, optimal policy array)
    """
...
def maxent_irl(env, feature_matrix, expert_expectations, start_probs=None, n_iterations=20, lr=0.1, horizon=40):
    """
    Executes the Maximum Entropy IRL algorithm to recover reward weights.
    
    Args:
        env (gym.Env): The underlying environment.
        feature_matrix (np.ndarray): Matrix of precomputed state-action features.
        expert_expectations (np.ndarray): Empirical feature counts from expert demos.
    ...
    for it in range(n_iterations):
        ...
        # --- Backward Pass (Log Space for Numerical Stability) ---
        ...
        for t in range(horizon - 1, -1, -1):
            for s in range(num_states):
                for a in range(num_actions):
                    ...
                    if not log_probs:
                        log_expected_next_z = -1e10 # Log(0) approx
                    else:
        ...
        # Calculate Expected Feature Counts & Gradient Update
...
if __name__ == "__main__":
    ...
    # 1. Ground truth weights and Expert Policy Generation
    ...
    # 2. MaxEnt Data Prep & Expert Trajectories
    ...
    # 5. Testing the Recovered Reward
